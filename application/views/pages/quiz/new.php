      ...
      <div class="form-group">
        ...
        foreach ($data['quiz_category'] as $key => $category) {
          ...
          <div class="custom-control custom-checkbox">
            <input type="checkbox" value="<?= $category['id'] ?>" name="category[]" class="custom-control-input" id="category-<?= $key ?>">
            <label class="custom-control-label" for="category-<?= $key ?>">
              <strong class="text-dark"><?= $category['name'] ?></strong>
              <input required type="number" class="form-control mt-2" value="0" autocomplete="off" name="total[]">
            </label>
          </div>
          <?php
        }
